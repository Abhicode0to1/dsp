...
function BotLinks({ links, onNavigate }) {
  ...
  return (
    <div className="flex flex-col gap-1 mt-2">
      {links.map((l, i) => (
        l.internal
          ? <button key={i} onClick={() => onNavigate(l.url)} className="flex items-center gap-1.5 text-xs text-indigo-600 hover:text-indigo-800 font-medium">
              <ChevronRight className="w-3 h-3" />{l.label}
            </button>
          : <a key={i} href={l.url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-xs text-indigo-600 hover:text-indigo-800 font-medium">
...
function BillingSummary({ data }) {
  if (!data) return null;
  return (
    <div className="mt-2 bg-indigo-50 rounded-lg p-2.5 text-xs space-y-1 border border-indigo-100">
          ...
          <div key={o.key} className="relative group">
            ...
            <div className="absolute bottom-full left-0 mb-1.5 w-56 bg-gray-900 text-white text-[11px] rounded-xl p-3 z-50 invisible group-hover:visible shadow-xl pointer-events-none group-hover:pointer-events-auto">
              ...
              <button onClick={onBilling}
                className="flex items-center gap-1 text-indigo-300 hover:text-white font-semibold transition-colors pointer-events-auto">
  ...
  return createPortal(
    <>
      ...
      {open && (
        ...
        >
          {/* Header */}
          <div className="bg-indigo-600 px-4 py-3 flex items-center gap-2.5 flex-shrink-0">
            <div className="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center flex-shrink-0">
              <MessageCircle className="w-4 h-4 text-white" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white leading-tight">Support Assistant</p>
              <p className="text-xs text-indigo-200 truncate mt-0.5">Ask me anything</p>
            </div>
            {msgs.length > 1 && (
              <button onClick={resetConversation} title="Start over"
                className="text-[10px] text-indigo-200 hover:text-white border border-white/20 hover:border-white/50 rounded-full px-2 py-0.5 transition-colors flex-shrink-0">
                ↩ Start over
              </button>
            )}
                ...
                <div key={i} className="flex justify-start">
                  <div className="max-w-[88%] bg-green-50 border border-green-200 rounded-xl p-3 text-xs text-green-800">
                    ...
                    <button onClick={() => goNavigate(`/customer/tickets/${m.ticket_id}`)}
                      className="mt-2 flex items-center gap-1 text-indigo-600 hover:text-indigo-800 font-semibold">
              ...
              if (m.role === 'user') return (
                <div key={i} className="flex justify-end">
                  <div className="flex flex-col items-end gap-0.5">
                    <div className="max-w-[85%] px-3 py-2 rounded-xl text-sm bg-indigo-600 text-white rounded-tr-sm">
                      {m.text}
                    </div>
                      ...
                      <div className="flex flex-col gap-1 mt-1 w-full">
                        {m.subtopics.map(t => (
                          <button key={t.label} onClick={() => sendQuery(t.msg, t.label)}
                            className="text-left text-xs px-3 py-1.5 rounded-lg border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 hover:border-indigo-400 transition-colors font-medium">
                            {t.label}
                          </button>
                        ))}
                      </div>
                      ...
                      <div className="flex flex-col gap-1 mt-1 w-full">
                        {SUBCATEGORIES[m.showSubcategories].map(sub => (
                          <button key={sub.label} onClick={() => sendQuery(sub.msg, sub.label)}
                            className="text-left text-xs px-3 py-1.5 rounded-lg border border-gray-200 bg-white text-gray-700 hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-700 transition-colors font-medium flex items-center gap-1.5">
                        ...
                        <div className="grid grid-cols-2 gap-1">
                          {CATEGORIES.map(c => (
                            <button key={c.label} onClick={() => handleCategory(c)}
                              className="text-left text-xs px-2.5 py-2 rounded-lg border border-gray-200 text-gray-600 hover:border-indigo-300 hover:text-indigo-600 hover:bg-indigo-50 transition-colors leading-tight">
                              {c.label}
                            </button>
                          ))}
                        </div>
          ...
          <div className="border-t border-gray-100 px-3 pt-2 pb-2.5 flex-shrink-0">
            ...
            {pendingFile && (
              <div className="flex items-center gap-1.5 mb-1.5 px-2 py-1 bg-indigo-50 border border-indigo-200 rounded-lg text-xs text-indigo-700">
                <Paperclip className="w-3 h-3 flex-shrink-0" />
                <span className="truncate flex-1">{pendingFile.name}</span>
                <button onClick={() => setPendingFile(null)} className="flex-shrink-0 text-indigo-400 hover:text-red-500 transition-colors">
                  <X className="w-3 h-3" />
                </button>
              </div>
            )}
            <div className="flex gap-2">
              ...
              {botPhase === 'collecting_ticket' && (
                <>
                  ...
                  <button onClick={() => fileInputRef.current?.click()} title="Attach file"
                    className="w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-lg border border-gray-200 text-gray-400 hover:text-indigo-600 hover:border-indigo-400 transition-colors">
              ...
              <input
                ref={pasteRef}
                type="text"
                className="flex-1 text-sm bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 transition-colors"
              ...
              <button
                ...
                className="w-8 h-8 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-lg flex items-center justify-center transition-colors flex-shrink-0"
              >
